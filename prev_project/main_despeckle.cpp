...
cv::Mat Openmp_DespeckleMedian(const cv::Mat& img, int kSize = 3) {
    ...
    for (int i = k; i < img.rows - k; i++) {
        for (int j = k; j < img.cols - k; j++) {
            ...
            neighborhood.reserve(kSize * kSize);

            for (int m = -k; m <= k; m++) {
                for (int n = -k; n <= k; n++) {
                    neighborhood.push_back(img.at<uchar>(i + m, j + n));
                }
            }
...
int main() {
    cv::Mat image = cv::imread("images/sample5.jpg");
    if (image.empty()) {
    ...
    // 10) Despeckling
    t1 = cv::getTickCount();
    cv::Mat despeckled = Openmp_DespeckleMedian(image, 5); // kSize=5
    t2 = cv::getTickCount();
    cv::imwrite("despeckled.jpg", despeckled);
    cv::imshow("Descpeckled", despeckled);
    cv::waitKey(0);
    cv::destroyAllWindows();
    ...
    std::cout << "10) Despeckled: " << (t2-t1)*1000/cv::getTickFrequency() << " ms" << std::endl;
    return 0;
}
